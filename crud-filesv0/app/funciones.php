...
function volcarDatostxt($tvalores){
    $fich = @fopen(FILEUSER,"w") or die("Error.");

    foreach ($tvalores as $users) {
        $linea = implode("|",$users)."\n";
        fwrite($fich,$linea);
    }
    fclose($fich);
...
function cargarDatoscsv (){
    ...
    $fichCsv = @fopen(FILEUSER, 'r') or die("ERROR al abrir fichero de usuarios");
    //bucle para recorer el fichero csv
    while (($datos = fgetcsv($fichCsv, 1000, ",")) !== FALSE) {
        $partes =$datos;
        $tabla[]= [ $partes[0],$partes[1],$partes[2],$partes[3]];
    }
    fclose ($fichCsv);
...
function volcarDatoscsv($tvalores){
    $fichCsv = @fopen(FILEUSER, 'w') or die("ERROR al abrir fichero de usuarios");

    foreach ($tvalores as $value) {
        fputcsv($fichCsv, $value);
    }
...
function cargarDatosjson (){
    $tabla=[];
    $datos = @file_get_contents(FILEUSER) or die("Error");
    $tabla = json_decode($datos,true);
    return $tabla;
}

function volcarDatosjson($tvalores){
    @file_put_contents(FILEUSER,json_encode($tvalores)) or die("Error");
}
